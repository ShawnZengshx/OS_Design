...
int kernel_main() {
	...
	*/
	...
	{
		JCB jcb;
		jcb.Init(i, (i + 1) * 50, Adata->time.RandNum(INS_NUM, INS_NUM));//为每个作业建立相应的作业控制块 
	...
	*/
	...
	/********************************先读出内存中的指令表*********************************/
	int ins_arr[3][INS_NUM] = {0};
	string files[3] = { "指令表1.txt" ,"指令表2.txt" ,"指令表3.txt" };
	for (int i = 0; i < 3; i++) {
	...
	Adata->pcb_table.PrintTable((char*)"进程表.txt");//打印进程表
	cout << "目前进程表信息已存入“进程表.txt”！" << endl;

	char* Npage[] = { (char*)"页表1.txt",(char*)"页表2.txt",(char*)"页表3.txt",(char*)"页表4.txt",(char*)"页表5.txt",(char*)"页表6.txt",(char*)"页表7.txt",(char*)"页表8.txt",(char*)"页表9.txt",(char*)"页表10.txt", };
	for (int i = 1; i <= JOB_NUM; i++)//打印页表
	{
	...
	while (Adata->ready.front != Adata->ready.rear)
	{
		...
		cout << "正在运行第" << e.ProID << "个进程" << "，第" << ins_arr[e.ProID-1][CPU.CurrentInsID-1] << "个指令,";
		Adata->frun << "正在运行第" << e.ProID << "个进程" << "，第" << ins_arr[e.ProID-1][CPU.CurrentInsID - 1] << "个指令,";

		if (Adata->pcb_table.process[e.ProID].instruc_arry[CPU.CurrentInsID - 1].State == 1)//当指令为用户态操作 
		{
			BlockQueue current_block_queue = Adata->jcb_table.jcb[e.JobId].block_queue; 		//获取当前的物理块队列 
			int addr = Adata->pcb_table.process[e.ProID].instruc_arry[CPU.CurrentInsID - 1].addr;
			...
			cout << endl;
			Adata->frun << endl;
			Sleep(Adata->pcb_table.process[e.ProID].instruc_arry[CPU.CurrentInsID - 1].time);
			Adata->pcb_table.process[e.ProID].RunTimes += Adata->pcb_table.process[e.ProID].instruc_arry[CPU.CurrentInsID - 1].time;
			alltime += Adata->pcb_table.process[e.ProID].instruc_arry[CPU.CurrentInsID - 1].time;
			CPU.CurrentInsID++;
			CPU.Sceneprotection(Adata->pcb_table.process[e.ProID]);

			Adata->run.DeQueue(e);
			if (CPU.CurrentInsID > Adata->pcb_table.process[e.ProID].InstrucNum)//判断是否执行完成 
			{
	...
	Adata->pcb_table.PrintTable((char*)"进程表(完成).txt");//打印进程表
